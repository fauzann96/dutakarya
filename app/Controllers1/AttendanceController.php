...
class AttendanceController extends BaseController
{
    ...
    public function input()
    {
        ...
        $view = view('attendance/attendance_input',$viewdata);
        return $view;
    }
    ...
    public function formData()
    {
        ...
        if($this->request->getPost('employee')!=0){
            ...
            $employeeData->where('customer_seq',$this->request->getPost('customer'));
        }
        ...
        $employeeData = $employeeData->findAll();
 

        $backupEmployeeData = $this->backupAssignModel->select('emp.id as id, emp.name, join_date, resign_date, emp.customer_seq, emp.deleted_at,tb_backup_assignment.id as backup,cust.name as emp_customer_name')->join('tb_employee emp','emp.id = tb_backup_assignment.employee_seq')->join('tb_customer cust','cust.id = emp.customer_seq');
        if($this->request->getPost('employee')!=0){
            $backupEmployeeData->where('tb_backup_assignment.employee_seq',$this->request->getPost('employee'));
        }else{
            $backupEmployeeData->where('tb_backup_assignment.customer_seq',$this->request->getPost('customer'));
        }
    ...
    public function inputSubmit()
    {
        $insert = '';
        $update = '';
        //cek sudah ada isinya belum?
        $errors = '';
        foreach ( $this->request->getPost('attendance_form_data') as $att_form_data_key => $att_form_data_item) {
            if(empty($att_form_data_item['employee_attendance_type'])){
                $errors = $errors.'Absensi #'.$att_form_data_key.' belum diisi, ';
            }
        }
        ...
        if($errors == ''){
            foreach ( $this->request->getPost('attendance_form_data') as $att_form_data_key => $att_form_data_item) {
                ...
                $data = [
                    ...
                    'customer_seq'=>$this->request->getPost('attendance_customer'),
                    'date'=>$this->request->getPost('attendance_date'),
                    'created_by'=> session()->get('user_id'),
                    'admin_seq'=> session()->get('user_id'),
                    'korlap_seq' => null,
                ];
    ...
    public function dataView(){
        ...
        $view = view('attendance/attendance_data',$viewData);
        return $view;
    }
    ...
    public function fetch(){
        ...
        ->where('tb_attendance_record.customer_seq',$this->request->getPost('customer'))
        ...
        //get backup employee ends
        //get each employee attendance record
    ...
    public function generateExcel($customer,$startDate,$endDate){
        $customerData = $this->customerModel->select('')->find($customer);
        ...
        // tulis header/nama kolom 
            ...
            foreach($employeeList as $employeeList_key =>$employeeList_value){
                ...
                if(!empty($employeeList_value['backup'])){
    ...
    public function generatePdf($customer,$startDate,$endDate)
    {
        $customerData = $this->customerModel->select('')->find($customer);
        if(empty($customerData)){
            echo "Permintaan tidak dapat diproses";
            ...
            $user = $this->userModel->select('name,signature')->find(session()->get('user_id'));
            ...
            $dompdf = new Dompdf;
            $html = view('attendance/attendance_generate_pdf',$viewData);
            //return $html;/*
